...
Page({
  ...
  async loadOrders(isPullRefresh = false, isLoadMore = false) {
    ...
    try {
      ...
      if (res.result && res.result.code === 200) {
        ...
        if (!isLoadMore) this._ordersLastLoadTime = Date.now();
      } else {
        wx.showToast({
  ...
  _filterByCategory(allOrders, selectedCategory) {
    ...
    if (selectedCategory === 'express') return allOrders.filter(o => o.orderType === 'express');
    if (selectedCategory === 'gaming') return allOrders.filter(o => o.orderType === 'gaming' || (o.storeName && o.storeName.includes('游戏')));
    if (selectedCategory === 'reward') return allOrders.filter(o => o.orderType === 'reward' || (o.storeName && o.storeName.includes('跑腿')));
    return allOrders;
  },
