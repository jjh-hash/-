...
Page({
    ...
    try {
      ...
      if (needStores) {
        try {
          ...
          if (storeRes.result && storeRes.result.code === 200 && storeRes.result.data.list) {
  ...
  applyProductSort(sortType, sourceProducts) {
    ...
    const stores = raw.filter(x => x.type === 'store');
    const products = raw.filter(x => x.type === 'product').map(p => ({ ...p }));
